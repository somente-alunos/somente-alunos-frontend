...
function Function_hasValidPrevisionDate(Parameter_content: Type_libraryDisplayContent): boolean {
	if (!Parameter_content.prevision_iso) {
		return false
	}

	return !Number.isNaN(new Date(Parameter_content.prevision_iso).getTime())
}

function Function_isLiveContent(Parameter_content: Type_libraryDisplayContent): boolean {
	return Parameter_content.is_available_now || Function_hasValidPrevisionDate(Parameter_content)
}

function Function_isGhostContent(Parameter_content: Type_libraryDisplayContent): boolean {
	return !Function_isLiveContent(Parameter_content)
}
...
export default function Page_Library(): JSX.Element {
	...
	const [isSyncActive, setSyncActive] = useState(false)
	...
	const syncFinishingRef = useRef(false)
	...
	const isFeaturedHasLiveContent = useMemo(() => {
		return isLibraryGroupedContent.featured.some(Function_isLiveContent)
	}, [isLibraryGroupedContent.featured])

	const isLiveUuidSet = useMemo(() => {
		return new Set(
			isLibraryContentDisplayArray
				.filter(Function_isLiveContent)
				.map((Parameter_single) => Parameter_single.content_uuid)
		)
	}, [isLibraryContentDisplayArray])
	...
	useEffect(() => {
		...
		const Const_hasNewLiveContent = Array.from(isLiveUuidSet).some(
			(Parameter_uuid) => !baselineLiveUuidsRef.current.has(Parameter_uuid)
		)
		if (Const_hasNewLiveContent) {
			Function_finishSync()
		}
	}, [isSyncActive, isLiveUuidSet, Function_finishSync])

	// Limpeza final ao desmontar a pagina.
	useEffect(() => {
		return () => {
			if (syncRafRef.current !== null) {
				cancelAnimationFrame(syncRafRef.current)
			...
			if (syncFinishTimeoutRef.current !== null) {
				clearTimeout(syncFinishTimeoutRef.current)
			}
		}
	}, [])
	...
	const Function_renderLibraryContentCard = (Let_contentSingle: Type_libraryDisplayContent): JSX.Element => {
		...
		const Const_isGhostSyncing = isSyncActive && Function_isGhostContent(Let_contentSingle)
		...
		return (
			<div
				...
				className={`rounded-md border pt-2.5 pb-4 px-2.5 transition-colors ${
					Const_isGhostSyncing
						? "border-new-primary-300 bg-new-primary-50/40 animate-pulse"
						: Const_canOpenViewer
							? `cursor-pointer border-default-400 ${isPressedLibraryCardUuid === Let_contentSingle.content_uuid ? "bg-default-300 border-default-500" : "bg-default-100/50"}`
							: "border-default-400 bg-default-100/50"
				}`}
				...
				<div className={`flex flex-wrap gap-2` + (!Const_canOpenPreview && !Const_canOpenFull ? " mt-4" : " mt-6")}>
					{Const_canOpenPreview ? (
						...
						<Button
							...
							color="default"
							variant="solid"
							className={`min-h-11 px-4 text-base font-semibold ${
								Const_isGhostSyncing
									? "bg-new-primary-100 text-new-primary-700"
									: "bg-default-300 text-default-800 active:bg-default-500"
							}`}
							startContent={Const_isGhostSyncing ? <Spinner size="sm" color="current" className="scale-75" /> : <Clock3 size={18} />}
							isDisabled
						>
							{Const_isGhostSyncing ? "Sincronizando…" : "Disponível em breve"}
						</Button>
					)}
				</div>
			</div>
					...
					<div className="mb-5 overflow-hidden rounded-2xl border border-new-primary-200 bg-gradient-to-br from-new-primary-50/80 to-new-secondary-50/60 p-4 shadow-sm">
						<div className="flex items-center gap-3">
							<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/80 text-new-primary shadow-sm">
								{isSyncProgress >= 100 ? (
									<Check size={20} className="text-new-secondary" />
								) : (
									<RefreshCw size={20} className="animate-spin [animation-duration:1.6s]" />
								)}
							</div>
							<div className="min-w-0 flex-1">
								<div className="text-[15px] font-semibold leading-tight text-default-900">
									{isSyncProgress >= 100 ? "Biblioteca sincronizada!" : "Sincronizando sua biblioteca…"}
								</div>
